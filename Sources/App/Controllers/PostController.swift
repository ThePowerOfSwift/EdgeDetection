...
final class PostController: ResourceRepresentable {
    ...
    func store(_ req: Request) throws -> ResponseRepresentable {
        print(req.headers)
        guard let imageStr = req.json?["image"]?.string else {
            return try JSON(node: ["status": "Error: Missing parameter - image"])
        }
   
        let edgeDetectionController = EdgeDetectionController()
        let base64Image = edgeDetectionController.detectEdge(imageStr: imageStr)

        if base64Image.range(of: "Error") != nil
        {
            return try JSON(node: ["status": base64Image as String])
        }
        else
        {
            return try JSON(node: ["status":"ok", "image": base64Image as String])
        }
    }
